...
# Where to go next. The four /health lines above only prove the services are
# listening; these are how you see the model actually run.
cat <<'NEXT'

== next ==
  python3 scripts/walkthrough.py            one Figure 3 step per screen
  python3 scripts/walkthrough.py --all      all five scenarios, in order
  python3 scripts/trace.py --last           all four transcripts, merged
  python3 conformance_probe.py --config team.json --verbose
  sh scripts/demo.sh                        the whole presentation runbook
NEXT
